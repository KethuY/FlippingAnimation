  ...
  private static class MyBaseAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
      ...
      if (previousTask != null) {
        if (previousTask.getPageIndex() == position && previousTask.getImageName()==data.imageRes) //check if the convertView happens to be previously used
        {
          needReload = false;
        } else {
      ...
      if (needReload) {
        AsyncImageTask
            task =
            new AsyncImageTask(photoView, controller, position,
                               data.imageRes);
       /* photoView
...
*/
  ...
  private static final class AsyncImageTask extends AsyncTask<Void, Void, Void> {
    ...
    public AsyncImageTask(ImageView imageView,
                          FlipViewController controller, int pageIndex, int imageName) {
...
return null;
      //return imageName ;//IO.readBitmap(assetManager, imageName);
    }

    @Override
    protected void onPostExecute(int bitmap) {
